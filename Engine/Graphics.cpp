...
void Graphics::draw_square(int x, int y, int size, Color c)
{
...
void Graphics::draw_reqt(int x, int y, int width, int height, Color c)
{
	...
	for (int i = -size/2; i <= size/2; i += 1) {
		for (int j = -size/2; j <= size/2; j += 1) {
			...
			if (abs(i) == size / 2 || abs(j) == size / 2) {
				...
				PutPixel(x + i, y + j, padding);
			}
			else {
				PutPixel(x + i, y + j, body);
